...
console.log('=== Buscando picks "Menos de" (Under) con línea >= 5.5 del día de hoy ===');
...
const targetPicks = todayPicks.filter(p => {
  ...
  if (parsed && parsed.type === 'total' && !parsed.over) {
    if (parsed.line !== undefined && parsed.line !== null && parsed.line >= 5.5) {
      return true;
    }
  }
  return false;
...
console.log(`Se encontraron ${targetPicks.length} picks de hoy "Menos de" con línea >= 5.5:`);
