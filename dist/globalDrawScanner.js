 ...
 */
function fairProbFromSnapshots(eventId, market, selection, oddDecimal) {
    try {
    ...
    `).all(eventId, market);
        if (legs.length > 1) {
            const probs = devig(legs.map((l) => l.odd_decimal), defaultMethod());
            const i = legs.findIndex((l) => l.selection === selection);
            if (i >= 0 && Number.isFinite(probs[i]) && probs[i] > 0)
                return probs[i];
        }
    }
    catch {
...
function scoreCandidate(c) {
    ...
    if (!row.oddDecimal || !(row.oddDecimal > 1))
        return null;
